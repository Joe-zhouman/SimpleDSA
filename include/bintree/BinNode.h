...
class BinNode {
    ...
    template <typename Visit>
    static void VisitAlongLeftBranch(BinNode<T>* x, Visit& vst,
                                     Stack<BinNode<T>*>& s);
    template <typename Visit>
...
public:
    ...
    explicit BinNode(T e, BinNode<T>* p = nullptr, BinNode<T>* lc = nullptr,
                     BinNode<T>* rc = nullptr, int h = 0, int l = 1,
                     Color c = Color::RED)
        : data_(e),
          parent_(p),
          left_child_(lc),
    ...
    int Size() {
        int s = 1;
        if (HasLeftChild()) s += left_child_->Size();
        if (HasRightChild()) s += right_child_->Size();
        return s;
    }

    BinNode<T>* InsertAsLeftChild(T const& e) {
        return left_child_ = new BinNode(e, this);
    }
    BinNode<T>* InsertAsRightChild(T const& e) {
        return right_child_ = new BinNode(e, this);
    }
    BinNode<T>* Succeed();
    ...
    template <typename Visit>
    void TraversalPre(Visit& vst) {
        TraversalLevel(vst, TraversalMethod::DEFAULT);
    }

    template <typename Visit>
    void TraversalIn(Visit& vst) {
        TraversalLevel(vst, TraversalMethod::DEFAULT);
    }

    template <typename Visit>
    void TraversalPost(Visit& vst) {
        TraversalLevel(vst, TraversalMethod::DEFAULT);
    }
    ...
    BinNode<T>* Uncle() {
        return parent_->IsLeftChild() ? parent_->parent_->left_child_
                                      : parent_->parent_->right_child_;
    }
};
template <typename T>
constexpr BinNode<T>* FromParentTo(BinNode<T>* x) {
    return x->IsRoot() ? x
                       : x->IsLeftChild() ? x->parent_->left_child_
                                          : x->parent_->right_child_;
}
template<typename T>
