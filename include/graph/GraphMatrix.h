...
namespace my_dsa {
	...
	public:
		...
		Tv& VertexData(int i) override {
			return v_set_[i].data_;
		}

		int InDegree(int i) override {
			return v_set_[i].in_degree_;
		}

		int OutDegree(int i) override {
			return v_set_[i].out_degree_;
		}

		int FirstNbr(int i) override {
			return NextNbr(i, this->v_);
		}
		...
		VStatusEnum& Status(int i) override {
			return v_set_[i].status_;
		}

		int& DTime(int i) override {
			return v_set_[i].d_time_;
		}
		int& FTime(int i) override {
			return v_set_[i].f_time_;
		}
		int& Parent(int i) override {
			return v_set_[i].parent_;
		}
		int& Priority(int i) override {
			return v_set_[i].priority_;
		}
		...
		bool Exist(int i, int j) override {
			return (0 <= i) && (i < this->v_) && (0 <= j) && (j < this->v_) && e_set_[i][j] != nullptr;
		}
		...
		ETypeEnum& Type(int i, int j) override {
			return e_set_[i][j]->type_;
		}
		Te& EdgeData(int i, int j) override {
			return e_set_[i][j]->data_;
		}
